...
cllming      include '../include/paramp.h'
      ...
      parameter(nx=3001,my=96,lev=1,itime=1)
...
c     +     status='unknown',access='direct',recl=nxmy4)
      ...
      open(22,file='zaw.dat',form='formatted',status='unknown')

      read(22,*),((enplot(i,j,1,1),i=1,nx),j=1,my)

      do i=1,nx,1
         do j=1,my,1
c               enplot(i,j,1,1)=enplot(i,j,1,1)*1.0e4
               enplot(i,j,1,1)=enplot(i,j,1,1)
         enddo
      enddo
...
c      data ishade /1,39,47/  ! for pv!
      data ishade /47,47,47,1,1,1/  ! for pv!
c      data ishade /1,35,39,43,47,51/  ! for pv!   llming
...
c---                                                                                
         ...
         call set(0.1,0.90,0.3,0.7,1.,121.,1.,96.,1)
         call pl2d(0.1,0.90,0.3,0.7,f(1,1),nx,my,rd,ncl,1,
     +             ishade,0,spval)
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++040420
c---- for center
...
c         call line(1.,1.,64.,64.)

         call gslwsc(1.)
         call set(0.,1.,0.,1.,0.,1.,0.,1.,1)
         call plchhq(.5,.75,'Zonal Average Zonal Wind',24.,0.,0.)
c         call plchhq(.5,.915,'m',24.,0.,0.)
cc        call plchhq(.86,.08,'vor',18.,0.,0.)

c         call plchhq(.15,.87,'0',18.,0.,0.)
c         call plchhq(.261,.87,'10',18.,0.,0.)
c         call plchhq(.372,.87,'20',18.,0.,0.)
c         call plchhq(.483,.87,'30',18.,0.,0.)
c         call plchhq(.594,.87,'40',18.,0.,0.)
c         call plchhq(.705,.87,'50',18.,0.,0.)
c         call plchhq(.816,.87,'60',18.,0.,0.)
         call plchhq(.1,.27,'   0',20.,0.,0.)
         call plchhq(.3,.27,' 750',20.,0.,0.)
         call plchhq(.5,.27,'1500',20.,0.,0.)
         call plchhq(.7,.27,'2250',20.,0.,0.)
         call plchhq(.9,.27,'3000',20.,0.,0.)
         call plchhq(.5,.225,'Time(Days)',20.,0.,0.)
...
c         call plchhq(.9,.27,'  0 ',18.,0.,0.)
         ...
         call plchhq(.02,.5,'Latitude',20.,90.,0.)
cc         call plchhq(.95,.088,'*10e-4',18.,0.,0.)

c         call plchhq(.12,.15,'0',18.,0.,0.)
c         call plchhq(.12,.261,'10',18.,0.,0.)
c         call plchhq(.12,.372,'20',18.,0.,0.)
c         call plchhq(.12,.483,'30',18.,0.,0.)
c         call plchhq(.12,.594,'40',18.,0.,0.)
c         call plchhq(.12,.705,'50',18.,0.,0.)
c         call plchhq(.12,.816,'60',18.,0.,0.)
