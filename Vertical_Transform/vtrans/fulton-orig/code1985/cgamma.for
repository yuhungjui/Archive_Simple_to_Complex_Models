...
C                       TOL.EQ.0    USE RELATIVE TOLERANCE  1.0E-15
C                                   (GIVES ESSENTIALLY MACHINE ACCURACY)
C
...
C
      ...
      REAL  ALPHA, BETA, COLD, CTOL, EPS, MU, NU, PI, ZC, ZT
      PARAMETER  ( NITMAX = 10 )
C
...
C
      IF ( ZT.GT.ZC )  THEN
          C(0) = SQRT( RTB*(1.0 - EXP( -ZT )) )
          MU = SQRT( 0.25 - RGAMMA/C(0)**2 )
          DO 10 I=1,NITMAX
              COLD = C(0)
              MU = MU - F0( MU )/F0P( MU )
              C(0) = SQRT( RGAMMA/(0.25 - MU*MU) )
              IF ( TOL.LE.0.0 )  CTOL = EPS*C(0)
              IF ( ABS( C(0) - COLD ).LE.CTOL )  GO TO 20
   10     CONTINUE
          IERR = -1
          RETURN
   20     PSIHAT(0) = SQRT( -2*MU*G0(0.0,MU)/(F0P(MU)*G0(ZT,MU)) )
          NSTART = 1
...
C
      ...
      DO 50 K=NSTART,N
          NU = K*PI/ZT
          C(K) = SQRT( RGAMMA/(0.25 + NU*NU) )
          DO 30 I=1,NITMAX
              COLD = C(K)
              NU = NU - FN( NU )/FNP( NU )
              C(K) = SQRT( RGAMMA/(0.25 + NU*NU) )
              IF ( TOL.LE.0.0 )  CTOL = EPS*C(K)
              IF ( ABS( C(K) - COLD ).LE.CTOL )  GO TO 40
   30     CONTINUE
          IERR = -(K + 1)
          RETURN
   40     PSIHAT(K) = SQRT( 2*NU*GN(0.0,NU)/(FNP(NU)*GN(ZT,NU)) )
   50 CONTINUE
      RETURN
      END
...
C
      ...
      INTEGER  I
      REAL  ALPHA, BETA, DP, MU, NU, ZC, ZT
C
C   GREEN'S FUNCTIONS FOR EXTERNAL AND INTERNAL MODES
C
      G0( Z, MU ) = COSH( MU*Z ) - (ALPHA/MU)*SINH( MU*Z )
...
C
      IF ( K.EQ.0 .AND. ZT.GT.ZC )  THEN
          MU = SQRT( 0.25 - RGAMMA/C(0)**2 )
          DO 10 I=1,NP
   10     PSI(I) = SQRT( DP/P(I) )*PSIHAT(0)*G0( LOG(PB/P(I)), MU )
      ELSE IF ( K.EQ.0 .AND. ZT.EQ.ZC )  THEN
          DO 20 I=1,NP
   20     PSI(I) = SQRT( DP/P(I) )*PSIHAT(0)*(1.0 - ALPHA*LOG(PB/P(I)))
      ELSE
          NU = SQRT( RGAMMA/C(K)**2 - 0.25 )
          DO 30 I=1,NP
   30     PSI(I) = SQRT( DP/P(I) )*PSIHAT(K)*GN( LOG(PB/P(I)), NU )
      END IF
      RETURN
      END
