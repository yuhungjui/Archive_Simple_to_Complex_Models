
FUNCTION shape_fun, s
shape_fun = (1-s)^2.0*(1+2*s)
RETURN, shape_fun
END 

;
; A PROGRAM TO PLOT DIFFERENT HEATING PROFILES
;
nvar=3
nr=1000

rad=FINDGEN(nr)*0.1

Q=FLTARR(nr,nvar)

r3=FLTARR(nvar)
r4=FLTARR(nvar)
r1=13
r2=17
r3(0)=58
r4(0)=62
r3(1)=43
r4(1)=47
r3(2)=28
r4(2)=32
m2 = 125.0   ; K/day

FOR j = 0, nvar-1 DO BEGIN
heat = (1.0/1000000.0)*(m2 * 28935.2) / $
       ( ((r3(j)+r4(j))/2.0)^2.0 - ((r1+r2)/2.0)^2.0 ) 
heat=heat/30.0
print, heat
FOR i = 0, nr-1 DO BEGIN
   IF rad(i) LT r1 THEN BEGIN
      Q(i,j) = 0.0
   ENDIF ELSE IF rad(i) LE r2 THEN BEGIN
      Q(i,j) = heat*shape_fun((r2-rad(i))/(r2-r1)) 
   ENDIF ELSE IF rad(i) LE r3(j) THEN BEGIN
      Q(i,j) = heat
   ENDIF ELSE IF rad(i) LE r4(j) THEN BEGIN
      Q(i,j) = heat*shape_fun((rad(i)-r3(j))/(r4(j)-r3(j)))
   ENDIF ELSE BEGIN
      Q(i,j) = 0.0
   ENDELSE
ENDFOR
ENDFOR

outfile1 = './masssink2.ps'
xaxis = "Radius (km)"
yaxis = "Mass Sink (s!E-1!N)"
titles = ['Mass Sink Profiles',xaxis,yaxis]
bounds = [0,80,0,0.0002] 
onedoverplotter2, Q, rad, TITLES, BOUNDS, outfile1

END
