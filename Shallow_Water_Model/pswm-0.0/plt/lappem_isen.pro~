;
; LAPPEM_ISEN.PRO
; Main plotting program for the Limited Area Periodic
; Primitive Equation Model - Isentropic Vertical Coordinate
;

PRINT, "Running LAPPEM_ISEN.PRO..."
...
READ, lbegin, PROMPT="Beginning file number: "
...
if (var EQ 'p') then begin
   ...
   clevels = [-200,-188,-176,-164,-152,-140,-128,-116,-104,-92,-80,-68,-56,-44,-32,-20,-8]
   p_colors = bwh_plot_colors
   ...
   titles  = ["v (m s!U-1!N)", "r (km)", "!7h!3 (K)"]
   clevels = [-20,-15,-10,-8,-6,-4,-2,-1,0,1,2,4,6,8,10,15,20]
   clevels = [0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8]
   p_colors = c2w_plot_colors
;   p_colors = bwl_plot_colors
endif else if (var EQ 'z') then begin
   titles  = ["!7f!3 (x 10!E-3!N s!E-1!N)", "r (km)", "!7h!3 (K)"]
   clevels = 0.1*[-20,1,3,5,7,10,12.5,15,17.5,20,25,30,35,40,50,60,70]
   p_colors = rbw_plot_colors
endif else if (var EQ 'd') then begin
   titles  = ["!7d!3 (x 10!E-3!N s!E-1!N)", "r (km)", "!7h!3 (K)"]
   clevels = 0.001*[-100,-50,-30,-20,-10,-5,-3,-1,0,1,3,5,10,20,30,50,100]
   p_colors = c2w_plot_colors
endif else if (var EQ 'q') then begin
   titles  = ["PV(r,!7h!3) (PVU)", "r (km)", "!7h!3 (K)"]
   titles  = ["PV (PVU)", "r (km)", "!7h!3 (K)"]
;  comment in for PV tower
;  clevels = 0.01*[-2,0,1,3,5,10,15,20,25,30,40,50,75,100,150,200,300]
   clevels = 0.01*[-200,50,60,70,80,90,100,125,150,175,200,225,250,275,300,325,350]
   p_colors = rbw_plot_colors
endif
...
if (azorslice LT 0.5) then begin
 ...
 ; bounds = [min(r),10000,min(thl),max(thl),timehour]

  twodplotter, field_azmean, r, thl, titles, p_colors, $
...
;             clevels, bounds, plotname
  ...
  if (pltyp eq 0) then begin
    ...
    bounds = [-100,100,-100,100,timehour]
;    clevels = min(qref) + FINDGEN(nlev) * (max(qref) - min(qref))/nlev
    titlesn = [titles(0), "x (km)", "y (km)"]

    twodplotter, qred, x, y, titlesn, p_colors, $
             clevels, bounds, plotname
...
;    clevels = min(qref) + FINDGEN(nlev) * (max(qref) - min(qref))/nlev
    titlesn = [titles(0), "x (km)", "!7h!3 (K)"]
;    clevels = -4.25 + FINDGEN(nlev)*0.5 
;
    twodplotter, qred, x, thl, titlesn, p_colors, $
             clevels, bounds, plotname
...
;    clevels = min(qref) + FINDGEN(nlev) * (max(qref) - min(qref))/nlev
    titlesn = [titles(0), "y (km)", "!7h!3 (K)"]

    twodplotter, qred, y, thl, titlesn, p_colors, $
             clevels, bounds, plotname
