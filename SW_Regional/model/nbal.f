...
c
      do 100 j=1,myr
      do 100 i=1,nxr
        w1(i,1,j)= (corr(i,j)+rvorr(i,1,j))*vtr(i,1,j)
        w2(i,1,j)=-(corr(i,j)+rvorr(i,1,j))*utr(i,1,j)
 100  continue
c
...
c      ws(1+jtrunxy,2,k)=0.
 53   continue
c
      call rtransr(jtrunx,jtruny,nxr,myr,levr,ws,ttr)
c
      do 120 j=1,myr
      do 120 i=1,nxr
        ttr(i,1,j)=ttr(i,1,j)-(utr(i,1,j)**2+vtr(i,1,j)**2)*0.5
c     &             +sgeor(i,j)  
 120  continue
c
      return
